...
- **Global scope**: `/home/diogo/dev/omo-profiler/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/home/diogo/dev/omo-profiler/.claude/notes/`)
...
2. Glob `/home/diogo/dev/omo-profiler/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/home/diogo/dev/omo-profiler/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/home/diogo/dev/omo-profiler/.claude/notes/` — works in any directory
