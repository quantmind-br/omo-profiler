...
| File | Role |
|------|------|
| `types.go` | Root `Config` struct + nested structs (45 top-level fields) |
...
- `Config`: Root container. 45 top-level fields including `Agents map[string]*AgentConfig`, `Categories map[string]*CategoryConfig`, `AgentOrder []string`
...
- `CodegraphConfig`: 6 fields — code-graph indexing subsystem (auto-init, auto-provision, install dir, telemetry, watch debounce) — added v4.11.0
